...
updates_dir=/data/euclid_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have euclid-VERSION-DATE-TYPE-* as format"
...
# Assume euclid-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.euclid.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.euclid.updater/databases/updates.db" \
